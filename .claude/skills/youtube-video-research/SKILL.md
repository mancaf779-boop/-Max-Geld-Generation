...
|---|---|---|
| Web search (general or news) | Discover YouTube videos and gather web context | Always; first step in every research task |
| Video analysis tool (any multimodal video-understanding tool/MCP server available in the session) | Analyze YouTube video content | After discovering relevant videos; the core analytical tool |
| Web search (research/academic mode, if available) | Find academic papers, industry reports for cross-validation | When rigorous sourcing is needed |
| Page fetch / browser tool | Read articles, transcripts, show notes linked from videos | When supplementary text context is needed |

If no dedicated video-analysis tool is available in the current environment, fall back to reading video descriptions, comments, and any published transcript via web search and page-fetch tools, and say explicitly in the report that analysis is based on text metadata rather than direct video review.
...
Use `scripts/build_research_brief.py` to turn a topic + research type (company/person/travel/generic) into a structured brief up front: it assembles the theme list, the query-variant patterns, the target video count, and the matching high-density analysis-prompt template from `references/prompt-templates.md`, so the discovery plan below is followed consistently rather than improvised per task. Run it as:

```bash
python3 scripts/build_research_brief.py "Stripe" --type company
python3 scripts/build_research_brief.py "Kyoto" --type travel --output brief.json
```
...
Use whatever video-analysis tool/MCP server is available in the session to extract insights from the selected videos.

#### Critical: High-Density Prompt Engineering

Video-analysis output quality is **entirely determined by prompt design**. A/B testing shows that a structured, quote-demanding prompt yields substantially more content, more direct quotes, and more precise data points compared to a generic "summarize" prompt — on the exact same video.
...
See [references/prompt-templates.md](references/prompt-templates.md) for domain-specific high-density prompt templates, or generate the right one automatically with `scripts/build_research_brief.py` (see Step 1).
...
1. **Pass 1 — Raw transcript**: Run a speech-to-text tool on the video (or fetch the platform's own captions/transcript) to get a verbatim text transcript.
2. **Pass 2 — Structured analysis**: Run the video-analysis tool with the high-density prompt for visual context, speaker identification, and structured extraction.
...
After analyzing videos, use web search (general or news) to:
...
- `scripts/build_research_brief.py`: deterministic helper that turns a `(topic, research_type)` pair into a structured research brief dict/JSON: the theme list for that research type (per Step 1 and the reference docs), a query-variant pattern per theme, the target video count band, and the matching analysis-prompt template pulled from `references/prompt-templates.md`. Supports `company`, `person`, `travel`, and a `generic` fallback for any other deep-research topic. Use it at the start of Step 1 to produce the concrete plan (and re-run it if the research type changes mid-task) rather than improvising the theme list from memory.
