...
For a fast, code-level quote + derived metrics (without going through the full profile/insights/chart workflow), use `scripts/fetch_quote.py`. It wraps `Yahoo/get_stock_chart`-style responses into a small client:

```bash
python3 scripts/fetch_quote.py AAPL --range 3mo --interval 1d
```

It returns the latest close, percent change over the requested range, and a simple moving average (SMA) computed from the historical closes, with clean error handling when a ticker is invalid or the API returns no data. Use this as the quantitative backbone before writing the narrative technical/fundamental sections; it does not replace `Yahoo/get_stock_insights` for analyst ratings and qualitative outlook.
