...
5. Guard checks after re-sync (see `tests/html_video_production/test_structure.py` for the automated versions): no hardcoded original-sandbox home-directory paths should remain; no references to the prior agent's internal user-notification tool should remain; and the prior agent's product name should not appear anywhere outside `LICENSE` / `NOTICE.txt` / `UPSTREAM_LICENSE_APACHE-2.0.txt` and this file's own provenance section (a substring match inside unrelated words like "manuscript" is not a violation).
