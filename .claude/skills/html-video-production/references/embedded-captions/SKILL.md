...
2. scaffold the project (**[CLAUDE CODE NOTE]** scaffold manually with `npm init -y && npm install hyperframes` — do **not** run `npx hyperframes init`, which re-pulls/overwrites the patched skills; skip even that if the project dir already exists with the video inside — `matte.cjs`/`transcribe.cjs` adopt any video in the dir as source.mp4) → **`bash scripts/prepare.sh <project>`** (matte ∥ transcribe ∥ audio-envelope in parallel, then safe-zones v2 with scene palette/optics/lighting — one command, nothing forgotten)
...
```
1. [CLAUDE CODE NOTE] scaffold manually (npm init -y && npm install hyperframes), then drop <video.mp4> into the project dir as source.mp4 — do NOT run `npx hyperframes init` (it re-pulls/overwrites patched skills)
