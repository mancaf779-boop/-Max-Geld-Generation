...
**[CLAUDE CODE NOTE]** Do **not** run `npx hyperframes init` (it re-pulls and overwrites these patched skills). Scaffold the project directory manually: create `videos/<project>/`, run `npm init -y` and `npm install hyperframes` there, and add a blank composition `index.html` by hand.

**[CLAUDE CODE NOTE — replaces the upstream auth-status preflight]** Run `npx hyperframes auth status` only if the user has asked to use a paid provider (HeyGen / ElevenLabs) and already has credentials configured; otherwise skip it and proceed directly to the brief. By default, voice is produced by local `npx hyperframes tts` and music by the local BGM generation recipe under the Two-Tier Priority Model in `../../_claude-overrides/media-generation.md`; don't surface a provider choice unless the user brings it up.

**Gate:** the project directory exists with `hyperframes` installed, and angle, length, aspect ratio, and language are locked. (No sign-in step.)
...
> **[CLAUDE CODE NOTE]** Do NOT run the command below. Generate this media with local tooling and hand-write the ledger instead — see `../../_claude-overrides/media-generation.md`.
