...
> **[CLAUDE CODE NOTE]** This entire credential/key section is **inert by default in this setup**. This skill does not set HeyGen / ElevenLabs / Lyria keys and does not run `npx hyperframes auth status`. Voice = `npx hyperframes tts`, music = the local BGM generation recipe, images = sourced from the user or the registry (Tier 1); SFX = bundled `assets/sfx/`, word timing = Whisper / `hyperframes transcribe`, background removal = `hyperframes remove-background` (Tier 2). The table below is retained for reference only. See [`../../../_claude-overrides/media-generation.md`](../../../_claude-overrides/media-generation.md).
