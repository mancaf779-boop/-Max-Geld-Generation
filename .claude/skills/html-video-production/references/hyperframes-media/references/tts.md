...
> **[CLAUDE CODE NOTE]** By default, **all voiceover is produced locally with `npx hyperframes tts` (Tier 1)** — only run `npx hyperframes auth status` or call HeyGen / ElevenLabs if the user has explicitly asked for one of those paid providers and already has credentials configured; don't present a voice-provider choice or a sign-in gate otherwise. Get word-level timestamps from the OpenAI Whisper API or `npx hyperframes transcribe` (Tier 2). The provider chain below is retained for reference — it applies only under the opt-in Tier 2 path. See [`../../../_claude-overrides/media-generation.md`](../../../_claude-overrides/media-generation.md).

`npx hyperframes tts` (upstream, **disabled by default in this setup**) auto-detects a provider from env vars; explicit override via `--provider`.
