...
The **music is the spine** — establish one track before anything else. This skill is tuned for **fast, high-energy BGM**: a strong beat grid drives the cuts (calm tracks work, but pace by phrase rather than beat). If the user gave you audio — a music file, or a video to pull the audio from — use it. If not, generate one: choose the mood from the user's description (e.g. "driving synthwave", "trap beat", "upbeat corporate") and **produce the track with the local BGM generation recipe (Tier 1)** per `../../_claude-overrides/media-generation.md`. **[CLAUDE CODE NOTE]** Do **not** run `npx hyperframes auth status`, do **not** present a sign-in / offline decision point, and never surface a BGM-provider choice (HeyGen / Lyria / MusicGen / ElevenLabs) — music is always the local BGM generation recipe. Save the generated track to `assets/bgm.mp3`. Stage any user-supplied images or videos so frames can weave them in on the beat grid; otherwise typography carries the whole video.

Initialize only if `hyperframes.json` is missing. Name `<project>` from the brief in kebab-case, such as `midnight-drive-loop` — never a timestamp. **[CLAUDE CODE NOTE]** Do **not** run `npx hyperframes init` (it re-pulls and overwrites these patched skills); scaffold `videos/<project>/` manually with `npm init -y` and `npm install hyperframes`.

```bash
# [CLAUDE CODE NOTE] manual scaffold instead of `npx hyperframes init`:
(cd "videos/<project>" && npm init -y && npm install hyperframes)
