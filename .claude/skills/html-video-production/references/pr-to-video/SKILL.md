...
**[CLAUDE CODE NOTE]** Do **not** run `npx hyperframes init` (it re-pulls and overwrites these patched skills). Scaffold `videos/<project>/` manually: `npm init -y`, `npm install hyperframes`, and add a blank composition `index.html` by hand.
...
**Gate:** the project directory exists with `hyperframes` installed; the PR ref is captured; angle, length, aspect ratio, and language are locked. (No sign-in step.)
