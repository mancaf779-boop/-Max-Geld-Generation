...
**[CLAUDE CODE NOTE — replaces the upstream auth-status preflight]** Run `npx hyperframes auth status` only if the user has asked to use a paid provider (HeyGen / ElevenLabs) and already has credentials configured; otherwise skip it and proceed directly. By default, voice is produced by local `npx hyperframes tts` and music by the local BGM generation recipe under the Two-Tier Priority Model in `../../_claude-overrides/media-generation.md`; don't surface a provider choice unless the user brings it up.

**Gate:** Site summary printed — strategy-first (what the product does, who it's for, brand voice) before the asset / color / font inventory. (No sign-in step.)
...
If Step 2 said no narration — produce background music with the local BGM generation recipe (no provider menu), then skip to Step 5. Otherwise: **[CLAUDE CODE NOTE]** generate narration with local `npx hyperframes tts` (never ask the user to pick a TTS provider), transcribe for word timestamps (Whisper API or `hyperframes transcribe`), map timestamps to beats, then ask about captions.
