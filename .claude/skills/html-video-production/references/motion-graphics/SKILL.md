...
# [CLAUDE CODE NOTE] manual scaffold instead of `npx hyperframes init` (init re-pulls/overwrites patched skills):
(mkdir -p "$PROJECT_DIR" && cd "$PROJECT_DIR" && npm init -y && npm install hyperframes)
```

**[CLAUDE CODE NOTE]** Do **not** run `npx hyperframes init` — it re-pulls the skill set from GitHub and overwrites these patched files. Scaffold manually as shown above.
