...
### scripts/multiclip_planner.py

Formalizes the arithmetic from Section 7 so it doesn't have to be done by hand:

```bash
python3 scripts/multiclip_planner.py plan-clips 260          # -> chunk durations, each <= 180s
python3 scripts/multiclip_planner.py crossfade-duration 120  # -> seconds for a 2-beat crossfade at 120 BPM
python3 scripts/multiclip_planner.py validate-arrangement prompt.txt 60  # -> checks timestamp sections match declared duration
```

Can also be imported directly: `from multiclip_planner import plan_clips, crossfade_duration, parse_timestamp_sections, validate_arrangement`.
