...
When comparing more than a couple of candidates, or when you have structured metadata (stars, last commit date, README presence, open issue count) for several repos at once, use `scripts/rank_repos.py` to classify and sort them into the Quality Tiers below instead of eyeballing the list. It implements exactly the thresholds documented in this file, so the tiering stays consistent across tasks.
...
Prefer higher tiers for reliability. "Active development" for the Promising tier and "well-documented"/"strong community" for Solid/Excellent are approximated in `scripts/rank_repos.py` by recency of last commit and README presence, since star count alone does not capture those qualities — see the script for the exact rule.
...
- `scripts/rank_repos.py`: given a JSON list of repo metadata dicts (`stars`, `last_commit_date`, `has_readme`, `open_issues`, and a `name`/`full_name` identifier), classifies each repo into Legendary/Excellent/Solid/Promising per the thresholds above and sorts the results by tier (best first) then by stars (descending). Use it whenever you're triaging more than a couple of candidate repos:

  ```bash
  python3 scripts/rank_repos.py candidates.json
  echo '[{"name": "yt-dlp", "stars": 90000, "last_commit_date": "2026-06-01", "has_readme": true, "open_issues": 1200}]' | python3 scripts/rank_repos.py -
  ```
