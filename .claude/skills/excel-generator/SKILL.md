...
### scripts/excel_style_kit.py

The theme system, data-block borders, column-width calculation, and sheet-index helpers shown as inline snippets above are also available as ready-made, tested functions — import them instead of retyping the snippets:

```python
from excel_style_kit import (
    THEMES, get_theme, apply_data_block_borders, calculate_column_width,
    add_sheet_index, style_title, style_section_header, style_table_header,
    apply_number_formats, SEMANTIC_COLORS, HIGHLIGHT_COLORS,
)

theme = get_theme('elegant_black')
style_title(ws, 'B2', 'Q3 Revenue Report', theme)
apply_data_block_borders(ws, start_row=7, end_row=9, start_col=2, end_col=5, has_header=True)
width = calculate_column_width(ws, col=2, data_block_ranges=[(7, 9)])
add_sheet_index(ws, ['Overview', 'Data', 'Analysis'], theme, start_row=6)
```

Run `python3 scripts/excel_style_kit.py demo <output.xlsx>` to generate a small sample workbook exercising the theme, borders, and sheet index in one file — useful for visually sanity-checking a theme before using it in a real report.
